...
LOG=$LOGS_DIR/ldsc_final.log
echo "=== PIPELINE FINAL (no merge-alleles) ===" | tee $LOG
...
for f in $MUNGE_DIR/big40_33k/*.sumstats.gz; do
    ...
    echo "rg [$((RG_DONE+1))/$S]: $base" | tee -a $LOG
